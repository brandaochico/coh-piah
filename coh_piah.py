...
def avalia_textos(textos, ass_cp):
    ...
    ass_txt = []
    for texto in textos:
        ass_txt.append(calcula_assinatura(texto))
        
    sim_menor = compara_assinatura(ass_txt[0], ass_cp)
    index = 0
    menor = 1
        
    while index < len(ass_txt):
        if compara_assinatura(ass_txt[index], ass_cp) < sim_menor:
            menor = index
